...
module.exports = function () 
{
    ...
    createLocalLogin = (server) =>
    {
        ...
        (
            ...
            {
                console.log('Local login: authenticating with:', username, password);
        ...
        server.post('/login', passport.authenticate('local', {failureRedirect: '/login/check'}), 
            function(req, res) 
            {
        ...
        server.post('/register-user', function(req, res, next) 
        {
            ...
            Credentials.findOne({email:email}).then((credentialsData) => 
            {
                if (!credentialsData)
                {
                    Credentials.create({email:email, password:hashedPassword}).then(newCredentials => 
                    {
